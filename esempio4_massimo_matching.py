...
# Inizializzazione
model = LpProblem("MassimoMatching", LpMaximize)
...
x = LpVariable.dicts("x", E, 0, None, LpContinuous)

# Funzione obiettivo
model += lpSum(x[e] for e in E) 

# Vincoli
for v in V:
    model += lpSum(x[e] for e in E if v in e) <= 1
...
if LpStatus[model.status] == "Optimal":
    ...
    print("\nMassimo matching = {}".format(round(value(model.objective),2)))
